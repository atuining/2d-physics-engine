...
AUTOTILE_MAP = {
    ...
    0,
    tuple(sorted([(1, 0), (0, 1), (-1, 0)])):
    1,
    tuple(sorted([(-1, 0), (0, 1)])):
    2,
    tuple(sorted([(-1, 0), (0, -1), (0, 1)])):
    3,
    tuple(sorted([(-1, 0), (0, -1)])):
    4,
    tuple(sorted([(-1, 0), (0, -1), (1, 0)])):
    ...
    tuple(sorted([(1, 0), (0, -1), (0, 1)])):
    7,
    tuple(sorted([(1, 0), (-1, 0), (0, 1), (0, -1)])):
    8,
}

NEIGHBOUR_OFFSETS = [(-1, 0), (-1, -1), (0, -1), (1, -1), (1, 0), (0, 0),
                     (-1, 1), (0, 1), (1, 1)]
...
class Tilemap:
    ...
    def __init__(self, game, tile_size=16):
        ...
        self.tilemap = {
        }  # only do physics here, easier to optimize if stuff is on a grid
        self.offgrid_tiles = []
    ...
    def render(self, surf, offset=np.array((0, 0))):
        ...
        for x in range(offset[0] // self.tile_size,
            ...
            for y in range(offset[1] // self.tile_size,
                           (offset[1] + surf.get_height()) // self.tile_size +
                           1):
                loc = str(x) + ';' + str(y)
